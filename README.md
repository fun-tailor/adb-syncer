...
如果你也是一个爱偷懒的用户，经常搬运一些固定文件，在 PC 和 Android 设备之间，不妨尝试下本软件，只需要插上 USB，等待若干秒，然后直接拔出。不需要点击鼠标，更无需关注屏幕，同步自然到位
...
##### 0.0.3:
- 增加 **托盘功能**，点 "x" 会缩小到托盘，双击托盘图标 恢复窗口，（note: 退出 只能通过托盘菜单）
- 增加 **"同步完成 显示通知"** 功能选项, 设置后 每次同步完成 会调用 windows 通知,（托盘右键菜单设置）

- adb 功能改变，应对 mime intent 激活问题，每次adb同步完，暂时调用 MEDIA_SCANNER_SCAN_FILE (SDK 29 deprecated 目前还有效)，待未来优化
...
pyinstaller --onefile --noconsole --name adb_syncer --add-data "res/icons;res/icons"  --hidden-import PyQt6.sip main.py
```
